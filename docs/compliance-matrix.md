...
2026-05-21 (xx.hocon#37 — E12 added) — no rate change. E12 (deferred substitution resolution — Lightbend-aligned `parse / withFallback / resolve()` lifecycle) added to [`extra-spec-conventions.md`](extra-spec-conventions.md#e12) as a cross-impl API convention with 14 normative decisions. External origin [go.hocon#99](https://github.com/o3co/go.hocon/issues/99) (@cgordon, third external issue). E-items do not contribute to the compliance denominator; rates unchanged in all three impls. New fixture group `testdata/hocon/deferred-resolution/` (30 scenario YAML files `dr01–dr30`) with paired Lightbend ground truth generated by new `DeferredResolutionRunner.java` (28 success/error scenarios verified against Lightbend; 2 `lightbendSkip`: dr11b for intentional Lightbend divergence per decision 10, dr17 for E11-not-applicable). Cross-spec interactions for S13a (self-reference lookback across fallback layers) and S10 (concat type-check under AllowUnresolved) clarified inline in the E12 section. Status 🤷 in all 3 impls — impl PRs target v1.4.0 bundle release alongside E11.
