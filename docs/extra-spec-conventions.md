...
**o3co convention**: reading (a). When `NAME` resolves to a config value **anywhere reachable via the standard substitution lookup path** — prefixed lookup against the merged config tree, plus original-path fallback per [S14c.2](spec-checklist.md) when the substitution originates from an included file — the `[]` suffix is stripped and the resolution returns the config value. Env vars `NAME_0`, `NAME_1`, ... are not consulted. This matches both the principle "config provided by the user always wins" that runs through HOCON (e.g. the `${?...}` optional-substitution semantics) and Lightbend 1.4.6's `ResolveSource.java::lookupSubst` which calls `expandEnvVarList` only when prefixed AND unprefixed config lookups both miss.
...
| Impl | Status | Test | Notes |
| --- | --- | --- | --- |
| ts.hocon | ✅ same-source / ⏳ cross-source (pending — tracked in [xx.hocon#22](https://github.com/o3co/xx.hocon/issues/22) C4) | `tests/env-var-list.test.ts` (ev05) loaded in [ts.hocon#100](https://github.com/o3co/ts.hocon/pull/100) | Resolver checks config presence before delegating to `resolveEnvList`; matches `${X}` same-source precedence. Cross-source (`${X[]}` in included file with X at root) requires the new S14c.2 original-path config fallback shipping in C4. |
| rs.hocon | ✅ same-source + cross-source | `tests/env_var_list_test.rs` (ev05) loaded in [rs.hocon#88](https://github.com/o3co/rs.hocon/pull/88); cross-source pinned by `lightbend_test03_includes_with_substitution_fallback` + ev12c sync | Config lookup wins; `resolve_env_list` consulted only when path is not config-defined in either prefixed or original-path scope. S14c.2 original-path fallback shipped in [rs.hocon#44](https://github.com/o3co/rs.hocon/pull/44). |
| go.hocon | ✅ same-source / ⏳ cross-source (pending — tracked in [xx.hocon#22](https://github.com/o3co/xx.hocon/issues/22) C2) | `s13c_env_var_list_test.go` (ev05) loaded in [go.hocon#86](https://github.com/o3co/go.hocon/pull/86) | Resolver has original-path config fallback for `${X}` non-list (S14c.2 ✅) but `${X[]}` branch currently runs BEFORE that fallback. C2 reorders the branches so list expansion only fires after both prefixed and original-path config lookups miss. |

**Fixtures**:

- Same-source: [`ev05-config-defined-wins.conf`](../testdata/hocon/env-var-list/ev05-config-defined-wins.conf) + expected. Generator pre-expansion uses `EnvVarListExpander.isDefinedInConfig` to detect the config-defined case before falling through to env-var lookup.
- Cross-source: [`ev12c-include-config-defined-wins.conf`](../testdata/hocon/env-var-list/ev12c-include-config-defined-wins.conf) (outer) + [`ev12c-inner.conf`](../testdata/hocon/env-var-list/ev12c-inner.conf) + expected. NO `.env` sidecar — routes through native Lightbend 1.4.6 (the EnvVarListExpander per-file pre-expansion cannot model cross-source config-key awareness). See [`fixture-conventions.md`](fixture-conventions.md) hybrid-mode note.
...
| sr13 | `foo.a = "x"; foo.a = ${?foo.a}bar; foo.b = ${foo.a}` | `{"foo":{"a":"xbar","b":"xbar"}}` | ❌ `xbarbar` | ❌ `xbarbar` | ✅ |
...
| Impl | Status | Test | Notes |
| --- | --- | --- | --- |
| ts.hocon | ✅ | sr12 ✅ / sr13 ✅ / sr14 ✅ / sr15 ✅ / sr16 ✅ ([ts.hocon#136](https://github.com/o3co/ts.hocon/pull/136)) | Fixed: `shouldFoldNested` gate removed, `resolveResObj` cache invalidation + escaped key (`stringSegmentsToKey`), `SubstPlaceholder.knownAbsent` sentinel + `foldOrSkipPrior` fold-to-sentinel. 20 unit tests added for `foldOptionalSelfRefAbsent` branches. |
| rs.hocon | ✅ | sr12 ✅ / sr13 ✅ / sr14 ✅ / sr15 ✅ / sr16 ✅ ([rs.hocon#124](https://github.com/o3co/rs.hocon/pull/124)) | Fixed: cycle handler + is_self_ref branch self-ref-loop guard (sr12); `resolve_res_obj` cache lifecycle + escaped key (sr14/16); `SubstPlaceholder.known_absent` sentinel + `#[non_exhaustive]` (sr15); `should_fold_nested` combined gate (Obj-Obj-subset OR Obj-non-Obj); resolver leaf-prior preferred over outer-nav for multi-segment self-refs (Round-3 regression fix). 10 unit tests for `fold_optional_self_ref_absent` branches. |
| go.hocon | ✅ | sr12 ✅ / sr13 ✅ / sr14 ✅ / sr15 ✅ / sr16 ✅ ([go.hocon#126](https://github.com/o3co/go.hocon/pull/126)) | Fixed: `knownAbsent` sentinel in `foldselfref.go` (sr15) + `findPrior(...)` consolidation for nested re-entrant path (sr13 regression from sr15); `containsKnownAbsentSentinel` / `rehydrateSentinel` extended to ArrayVal/ObjectVal containers with single-element rehydration (NOT splice-flatten — matches Lightbend immediate-resolve behavior). 45 unit tests for sentinel helpers + branch coverage. |
...
**The correctness contract is the round trip, not the byte format**: for a resolved, data-only config, `parse(render(tree))` MUST yield a value tree identical to `tree`. Byte-for-byte output is deliberately unpinned so implementations can improve formatting without breaking parity; the shared corpus below compares trees, never text.

Decisions (all four implementations MUST agree):

1. **Input domain**: a resolved config holding only data (objects / arrays / string / number / boolean / null) — exactly what `FromMap` and the format adapters produce. An unresolved placeholder is an **error**; substitutions have no textual round trip through a value tree. Source comments are not represented.
2. **Layout**: the root object's fields are emitted without enclosing braces; nested objects as `key { … }` (never dotted paths); arrays newline-separated inside `[ … ]`; two-space indent. Empty containers render `{}` / `[]`.
3. **Keys**: bare iff the key matches `^[A-Za-z0-9_-]+$` AND is not the word `include`; otherwise the JSON-quoted form. (Dots must quote — a bare dot would nest; bare `include` at the start of a key is reserved by every conformant parser, Lightbend included, so it must quote to round-trip.)
4. **Strings**: bare iff the value matches `^[A-Za-z][A-Za-z0-9_-]*$` AND is not a boolean/null keyword (`true/false/null/yes/no/on/off`, case-insensitive); everything else is quoted so it re-parses as a string — `"8080"` stays a string, `${…}` look-alikes stay literal (S13.5). Numbers, booleans and null emit raw.
5. **Triple-quoting**: a string containing LF is triple-quoted only when that is unambiguous — no CR anywhere, no embedded `"""`, and no trailing `"`; otherwise it falls back to the escaped double-quoted form (`\" \\ \n \r \t`, all other characters verbatim). Every parser now preserves CR verbatim inside triple quotes per S9.2 (go normalised CRLF to LF until 2026-08-19, matching neither Lightbend nor its siblings — fixture tq04 pins the verbatim behaviour), so the no-CR guard is a readability choice, not a losslessness requirement: an invisible raw CR inside triple quotes is ambiguous to human readers and editors. Note the leading-newline case round-trips **only because of the S9.2 fix** (the pre-2026-08-18 ts/py/rs lexers stripped it).
6. **API naming**: go `RenderHOCON()`, ts `renderHocon()`, py `render_hocon()`, rs `render_hocon()`.

**Round-trip corpus**: [`testdata/emitter-roundtrip/`](../testdata/emitter-roundtrip/) (rt01–rt10) holds JSON value trees; each implementation's runner loads a tree via its `FromMap` equivalent, renders, re-parses, and compares canonical trees. Numbers in the corpus stay within 2^53 so every host parses the JSON identically; integer-bound behaviour is per-impl-tested instead. The corpus deliberately includes the sharp edges: leading-newline / trailing-quote / embedded-`"""` / CR strings (decision 5), keyword and numeric strings (decision 4), quoted-key shapes incl. the empty key (decision 3), and substitution look-alikes.
...
2026-05-25 — E14 status roll-up to ✅ across all 3 impls. Per-impl PRs merged: [ts.hocon#136](https://github.com/o3co/ts.hocon/pull/136), [rs.hocon#124](https://github.com/o3co/rs.hocon/pull/124), [go.hocon#126](https://github.com/o3co/go.hocon/pull/126). 3 rounds of multi-agent-review (Claude + Codex) per PR caught 4 cross-impl regression classes: (a) dotted-key cache collision in resolveResObj (rs+ts), fixed via segment-key escape; (b) `shouldFoldNested` correctness gate (ts → rs follow-up): non-Obj overwrites must fold nested self-refs, same-key Obj-Obj subset must too, but not Obj-Obj-adding-new-keys (sr13 double-fold guard); (c) sentinel scope (go): `containsKnownAbsentSentinel` / `rehydrateSentinel` must recurse into ArrayVal/ObjectVal containers, with single-element rehydration (NOT splice — matches Lightbend immediate-resolve semantics, verified via go probe); (d) leaf-prior preference over outer-nav for multi-segment self-refs (rs): when both inner leaf prior and outer-folded prior exist, leaf takes precedence to avoid double-fold. sr13 was un-verified pre-fix; verified during fix work as ts/rs `xbarbar` (Bug #1 + prior combo), go ✅ (pointer-identity structurally robust). All 5 sr fixtures now pass across all 3 impls. Branch coverage: ts 100% / rs/go ~95-97% on the new fold/sentinel helpers (45 new unit tests in go, 20 in ts, 10 in rs).
