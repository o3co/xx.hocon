...
3. **Keys**: bare iff the key matches `^[A-Za-z0-9_-]+$`; otherwise the JSON-quoted form. (Dots must quote — a bare dot would nest.)
...
5. **Triple-quoting**: a string containing LF is triple-quoted only when that is lossless — no CR anywhere (the parser normalises CRLF), no embedded `"""`, and no trailing `"`; otherwise it falls back to the escaped double-quoted form (`\" \\ \n \r \t`, all other characters verbatim). Note the leading-newline case round-trips **only because of the S9.2 fix** (the pre-2026-08-18 ts/py/rs lexers stripped it).
