...
| Impl | Status | Test | Notes |
| --- | --- | --- | --- |
| ts.hocon | ✅ same-source / ⏳ cross-source (lands via [ts.hocon C4 PR](https://github.com/o3co/ts.hocon/issues)) | `tests/env-var-list.test.ts` (ev05) loaded in [ts.hocon#100](https://github.com/o3co/ts.hocon/pull/100) | Resolver checks config presence before delegating to `resolveEnvList`; matches `${X}` same-source precedence. Cross-source (`${X[]}` in included file with X at root) requires the new S14c.2 original-path config fallback shipping in C4. |
...
| go.hocon | ✅ same-source / ⏳ cross-source (lands via [go.hocon C2 PR](https://github.com/o3co/go.hocon/issues)) | `s13c_env_var_list_test.go` (ev05) loaded in [go.hocon#86](https://github.com/o3co/go.hocon/pull/86) | Resolver has original-path config fallback for `${X}` non-list (S14c.2 ✅) but `${X[]}` branch currently runs BEFORE that fallback. C2 reorders the branches so list expansion only fires after both prefixed and original-path config lookups miss. |
