...
2. Receiver `a = ${?a} extra`, fallback has no `a`. After merge + resolve: `a = " extra"` (optional self-ref to undefined → empty contribution per S13a.4).
3. Receiver `a = ${a} extra` (required), fallback `a = base`. After merge + resolve: `a = "base extra"`.
4. Receiver `a = ${a} extra`, fallback has no `a`. After merge + resolve: **CycleError** (the substitution looks at the very value it is defining; per dr06 this surfaces as a cycle in Lightbend; impls without a distinct cycle category MAY surface as `ResolveError`).
