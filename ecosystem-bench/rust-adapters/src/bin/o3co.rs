//! Ecosystem-bench adapter for hocon-parser (o3co/rs.hocon). Parses + resolves
//! a HOCON file and prints the resolved tree as JSON to stdout. On any
//! parse/resolve error it prints an error record to stdout and exits non-zero,
//! so the driver can score error-vs-success uniformly.
//!
//! Usage: `<conf-file>` (built via `cargo build --release --bin o3co`).
//!
//! Output is built through `serde_json`, which guarantees valid, fully-escaped
//! JSON. The driver ignores key order during normalization.
//!
...
use o3co_hocon::{Config, HoconValue, ScalarType};
...
fn main() {
    ...
    match o3co_hocon::parse_file(&args[1]) {
